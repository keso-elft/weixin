					...
					public void run() {
						while (true) {
							try {
								...
								Map<String, String> cookie = SendTools.auth("keso.elft@gmail.com", "");

								// 获取粉丝列表
								List<WeiXinFans> list = SendTools.getFans(cookie);

								// 群发
								for (WeiXinFans fans : list) {
									SendTools.sendMsg(cookie, content, fans.getFakeId());
								}
